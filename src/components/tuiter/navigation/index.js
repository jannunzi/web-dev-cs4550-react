...
function Navigation() {
  ...
  const links = [
    {label: 'Tuiter', icon: 'fa-square-t', path: ''},
  ...
  return(
    <div className="ttr-navigation">
      ...
      links.map((link, ndx) => {
        ...
        <li key={ndx} className={`list-group-item border-0 ttr-font-size-150pc text-nowrap
         ${pathname.indexOf(link.path) >= 0 ? 'fw-bold':''}`}>
          <Link to={`/tuiter/${link.path}`} id={link.label}
     ...
     <a href="#"
        className="mt-3 btn btn-lg btn-primary rounded-pill w-100 fw-bold text-white ps-0 pe-0">
      Tuit</a>
    </div>
  );
};
